...
- 数据隔离：训练使用 800 条、8 个说话人；checkpoint 选择只使用 100 条
  validation `s6`；所有模型冻结后才一次性评价 100 条 test `s7`。
...
test `s7` 已用于本轮一次性评价，后续不得根据这些数值选择超参数。下一项工作应
只在 train/validation 上做最小选择器消融，优先判断速度损失和 SNR 条件输入对
K=6/8 退化的影响；若产生新方案，最终确认应使用新的未触碰身份或预先冻结的
多折说话人协议。本轮尚未对 48,000 个条件运行 LivePortrait，也未计入 Top-K
索引传输、量化或真实比特率。
