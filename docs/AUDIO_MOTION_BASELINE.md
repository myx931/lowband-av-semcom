...
按预先定义的单一门槛，validation 平均 L1 比 `train_mean` 低约 0.30%，E4 技术
门槛通过；test 平均 L1 比 `train_mean` 低约 4.77%。但 test 上 GRU 比
...
E3 已证明数据隔离、因果建模、三种子训练、独立评价和冻结重建评价能够完整闭环。
不过，当前每个 split 只有一个说话人，validation 优势很小且 test 未超过
`zero_motion`。在开展预测残差价值实验前，应优先检查音频—运动时间对齐，并扩充
训练说话人；否则残差实验可能主要测量欠拟合预测器，而不是音频可解释运动的真实
边界。
