...
这份文档只引用已经冻结且有效的 E2–E8 结果，不引用作废的首次 E3 时间拉伸
...
先看 `summary.json` 和 `main_comparison.csv`；需要论文图时看 `figures/`，需要
逐样本区间时看 `e7_motion_bootstrap.csv` 与 `e7_video_bootstrap.csv`，需要
可直接阅读的章节时看
[中文结果章节草案](../THESIS_RESULTS_CHAPTER_DRAFT_CN.md)。
...
E8 已完成。运动结果先在每条样本内平均三个噪声实现，再以 100 条 test 样本做
10,000 次配对 bootstrap；视频按同一样本和名义噪声种子 42 配对。

| 指标 | residual 点估计胜出 | residual 区间胜出 | full-motion 区间胜出 | 跨零 |
| --- | ---: | ---: | ---: | ---: |
| 运动 L1 | 14/16 | 11/16 | 1/16 | 4/16 |
| 嘴部 NME | 13/16 | 12/16 | 1/16 | 3/16 |

优势定义为 `full-motion error - residual error`。正值支持 residual。区间是 95%
描述性点态区间，没有多重比较校正。

运动 L1 唯一明确支持 full-motion 的条件是 `C=2,10 dB`；嘴部 NME 唯一明确
支持 full-motion 的条件是 `C=4,10 dB`。所以论文应写“低/中 SNR 和紧预算下
优势更稳定”，不能写“所有条件都更优”。

固定定性样本为 `s7_bbae6n`、`s7_bbwl8n` 和 `s7_bgim6p`，对应字典序零基位置
0/50/99，不允许按观感重新挑选。

## 10. 下一步计划：E9 论文成稿

1. 将 E8 的 6 张正文图、1 张附录统计图和统一主表嵌入学校论文模板；
2. 使用[中文结果章节草案](../THESIS_RESULTS_CHAPTER_DRAFT_CN.md)完成方法和结果
   章节交叉引用；
3. 将“复信道符号”、指标方向、oracle 边界和 test 使用规则统一到所有图注；
4. 制作答辩版四图叙事：音频基线、残差 JSCC、低 SNR 门控、匹配对照；
5. 只在论文结构发现明确缺口时，再计划新的未触碰说话人或跨数据集实验。
