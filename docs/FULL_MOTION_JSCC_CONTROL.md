...
完整运动的 LivePortrait 正式评价已经完成：100 条 test 样本、每条 17 个条件，
共 1,700 条视频指标，失败数为 0。每条包含 1 个共同 oracle 和 16 个
`C×SNR` 条件；audio prediction 复用 E5 的同一候选，无噪声自编码器不重复
渲染。两种表示的嘴部关键点检测覆盖率在全部 16 个聚合条件中均为 100%。

最终冻结比较位于
`outputs/full_motion_vs_residual/20260728T151133.243833Z`，包含 4,800 个
逐样本/噪声运动配对、16 个运动组和 16 个视频组。下表的正值表示 residual
误差更低，负值表示 full-motion 更低。

嘴部 ROI MAE 相对优势：

| SNR | C=1 | C=2 | C=3 | C=4 |
| ---: | ---: | ---: | ---: | ---: |
| -5 dB | +24.25% | +22.58% | +16.36% | +13.97% |
| 0 dB | +17.94% | +14.05% | +12.89% | +11.51% |
| 5 dB | +13.05% | +4.22% | +8.06% | +6.53% |
| 10 dB | +10.78% | -1.62% | +4.08% | +1.86% |
...
residual 在运动 L1 的 16 个组中胜出 14 组，在嘴部 ROI MAE 中胜出 15 组，
在嘴部 NME 中胜出 13 组。视频结果与运动结果的主趋势一致：低/中 SNR 下
residual 优势稳定且通常更大；10 dB 和较大预算下差距缩小，full-motion 在
少数 NME 条件中更好。`C=1` 的 residual 在四个 SNR 上始终更好，说明音频
先验对紧预算尤其有价值。

## 结论与边界

E7 对 RQ2 提供了直接但有条件的支持：在匹配网络规模、复符号预算和 AWGN
设置下，预测残差在受限或较噪信道中通常比完整 18 维运动更有效；高 SNR、
较大预算时完整运动可以追平或局部胜出。因此论文不应声称“残差始终最优”，而应
把贡献限定为利用音频侧信息降低紧资源条件下的视觉语义恢复难度。

本比较没有定义数字 bitstream，仍只能报告复符号/段和复符号/秒。名义噪声种子
匹配，但当 validation 选中的模型种子不同时，精确逐元素噪声实现并不完全相同；
汇总结论基于冻结 test 的 100 条样本聚合，不声称严格配对噪声显著性。
